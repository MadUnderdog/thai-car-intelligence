...
import { mergeEvidence, getVectorEvidence } from "../../../../../lib/ai/retrieval/evidence-merge";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 2. Vector evidence retrieval (supplemental)
    const vectorResult = await getVectorEvidence(parsed.data.question);

    // 3. Build structured evidence from catalog
    const structuredEvidence = variants.map((v) => ({
      id: `variant:${v.id}`,
      kind: "fact" as const,
    ...
    // 5. Check if we have any evidence
    ...
    // 8. Use AI provider for enhanced response
